...
namespace Renderer{
	...
	class PhotonMappingEngine{
		...
		double initRadius;
		public:
		PhotonMappingEngine(Bitmap& bitmap,const Scene& scene,const Camera& camera,double initRadius=1)noexcept:bitmap(bitmap),scene(scene),camera(camera),initRadius(initRadius),ranGen(::std::time(NULL)){}
