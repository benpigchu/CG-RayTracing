...
namespace Renderer{
	...
	class PhotonMappingEngine{
		...
		double photonNum=0;
		public:
		PhotonMappingEngine(Bitmap& bitmap,const Scene& scene,const Camera& camera,double initRadius=5)noexcept:bitmap(bitmap),scene(scene),camera(camera),initRadius(initRadius),ranGen(::std::time(NULL)){}
